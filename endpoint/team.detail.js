...
module.exports = {
  Read(req, res, next) {
    ...
    query('select team.teamProfileName, user.userProfileName as teamOwnerUserName, case user.userProfileImg is null when 1 then user.userProfileImgDefault else user.userProfileImg end as teamOwnerUserImg, count(teamMember.userID=?)>0 as isTeamMember from team left join teamMember on team.teamID=teamMember.teamID left join user on team.teamOwnerUserID=user.userID where team.teamID=?', [
      userID, teamID
    ])(result => {
